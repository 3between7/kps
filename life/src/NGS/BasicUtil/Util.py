...
#         del self.winValueL[:]
        ...
        firstComeInWin = True
        notjustforsnp = True
        for findfirstidx in range(len(L)):#去掉窗口左边的位点
            if L[findfirstidx][0]>winStart:#若第一个SNP位点的位置在窗口右侧
                currentIdx=findfirstidx
                break
        while currentIdx != len(L) and L[currentIdx][0]<= L_End_Pos:#当SNP仍然位于该染色体上时
#             print(L[currentIdx][0],L[currentIdx])
            if L[currentIdx][0] > winStart and  L[currentIdx][0] <= (winStart + windowWidth):#如果该snp的位置在窗口里的话
#                if notjustforsnp or (len(L[currentIdx][1])==1 and re.search(r'[^a-zA-Z]', L[currentIdx][2]) != None and len(L[currentIdx][2])==1):# it's not a snp? indel or cnv
                ...
                if FoundNextIdx == False and L[currentIdx][0] > (winStart + slideSize):  # always go to |currentIdx+=1| #若位点已经超出窗口右侧边缘
                    nextIdx = currentIdx
                    FoundNextIdx = True
            else:#该SNP位点若在窗口外
